...
\Note{
    A binary relation $F$ is called a \Keywords{function} (or \Keywords{mapping}, \Keywords{correspondence}) if \Keywords{$a F b_{1}$ and $a F b_{2}$} imply \Keywords{$b_{1}$ $=$ $b_{2}$} for any $a$, $b_{1}$, and $b_{2}$.
}
\Note{
    \Keywords{Function} is a \Keywords{procedure}, a \Keywords{rule}, \Keywords{assigning to any object $a$ from the domain of the function a unique object $b$, the value of the function at $a$}.
}
\Note{
    A \Keywords{function} represents a special type of \Keywords{relation}, a relation where every \Keywords{object $a$ from the domain} is related to precisely \Keywords{one object in the range}, namely, to the value of the function at $a$.
}

\Proposition{}{
    Let $F$ and $G$ be functions.
    \Keywords{$F$ $=$ $G$} if and only if \Keywords{$\domain{F}$ $=$ $\domain{G}$} and \Keywords{$F(x)$ $=$ $G(x)$} for all $x$  $\in$ $\domain{F}$.
}{
...
\Proposition{}{
    Let $f$ and $g$ be functions.
    Then \Keywords{$g {\composition} f$ is a function}, \Keywords{$g {\composition} f$ is defined at $x$ if and only if $f$ is defined at $x$ and $g$ is defined at $f(x)$}.
    \List{enumerate}{
        \item{
...
\Proposition{}{
    If $f$ is a function, its \Keywords{inverse} $\Inverse{f}$ is a \Keywords{relation}, but it may not be a function.
}{
    Obviously.
}
...
\Theorem{}{
    If $F$ is a \Keywords{compatible system of functions}, the $\bigcup F$ is a function with $\domain(\bigcup F)$ $=$ $\bigcup\SetComprehension{\domain{f}}{f \in F}$.
